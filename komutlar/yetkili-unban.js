...
exports.run = (client, message, args) => {
  ...
  let user = args[0];
  let modlog = guild.channels.cache.find('name', '「⚙」log-kanalı');
  if (!modlog) return message.reply('`mod-log` kanalı mevcut değil.');
  if (reason.length < 1) return message.reply('**Banını Kaldıracağın Kişinin İd"sini Ve Sebebini Yazmalısın**');
