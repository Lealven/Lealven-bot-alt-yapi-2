...
exports.run = async(client, message, args) => {
  ...
  if (!args[0]) return message.channel.send(`**Küfür-Engel Filtresini Ayarlamak İçin \`-küfür-engel aç\` **                                                                                 ** Kapatmak İstiyorsanız \`-küfür-engel kapat\`   Yazabilirsiniz**`)
  if (args[0] !== 'aç' && args[0] !== 'kapat') return message.channel.send(`Küfür Filtresini Ayarlamak İçin \`-küfür-engel aç\` | Kapatmak İstiyorsanız \`-küfür-engel kapat\` Yazabilirsiniz`)
